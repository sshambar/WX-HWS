	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData[5] >=0)
					dataPoints2.push({label:moment(rowData[1]).format('HH:mm'),y:parseFloat(rowData[5])});
      ...
      for (var i = 0; i < e.entries.length; i++) {
        var temp = '<span style="color: ' + e.entries[i].dataSeries.color + ';">' + e.entries[i].dataSeries.name + '</span> <span style="color: <?php echo $fontcolor;?>;">' + e.entries[i].dataPoint.y.toFixed(1) + ' UVi</span> <br/>';
        str = str.concat(temp);
      }
      return (str);
    },
...
<body>
<div class="weather34darkbrowser" url="UV Index - <?php echo date('l') ;?> &nbsp;&nbsp;|&nbsp;&nbsp; High: <?php echo $weather['uv_max'];?> UVi"></div>
